...
const ImageComponent = props =>{
  ...
  const addTag = () =>{
    ...
    API.put('','',{queryStringParameters: params})
    .then(response => {
  ...
  const deleteImageFromDB = () =>{
    ...
    API.del('','',{queryStringParameters: params})
    .then(response => {
      deleteImageFromS3()
    })
    .catch(error =>{
...
const AuthStateApp = () => {
    ...
    const loadImageETag = async() =>{
      ...
      API.get('','',{queryStringParameters: params})
      .then(response => {
        setETags(response);
    ...
    const upload = async() => {
      ...
      await Storage.put(`${user.username + Date.now()}.jpeg`, uploadImage, {contentType: 'image/jpeg'});
